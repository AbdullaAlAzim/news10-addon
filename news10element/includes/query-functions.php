...
function magnews_orderby_post(){
    return array(
        'none' => __('No order', 'news10element'),
        'ID' => __('Post ID', 'news10element'),
        'author' => __('Author', 'news10element'),
        'title' => __('Title', 'news10element'),
        'date' => __('Published date', 'news10element'),
        'modified' => __('Modified date', 'news10element'),
        'parent' => __('By parent', 'news10element'),
        'rand' => __('Random order', 'news10element'),
        'comment_count' => __('Comment count', 'news10element'),
        'menu_order' => __('Menu order', 'news10element'),
        'post__in' => __('By include order', 'news10element'),
    );  
}
...
function madmag_ft_images($thumbnail){
    ...
    $placeholder =  News10_PLUGIN_URL . 'assets/lazyload.svg';
...
function news10_get_category_link($taxonomy){
...
function news10_user_list(){
 $args = array(
    ...
    $list = array();
    foreach ($posts as $cpost){
        $list[$cpost->ID] = $cpost->display_name;
    }
    return $list;
}
...
function misha_loadmore_ajax_handler(){
    ...
    require News10_PLUGIN_DIR.'includes/loops/'. $template .'.php';
    die(); 
}
...
function misha_tabcat_ajax_handler(){
    ...
    //require(News10_PLUGIN_DIR.'includes/loops/tabcat.php');
    require News10_PLUGIN_DIR.'includes/loops/'. $template .'.php';
    die(); 
}
