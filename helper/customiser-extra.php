...
class news10_plugins_hooks {

    function __construct() {
 
        add_action('wp_body_open',array(&$this,'render_preloader'));
        add_action('wp_body_open',array(&$this,'render_scroll_top'));
        add_action('upload_mimes',array(&$this,'news10_mime_type'));
        add_action('wp_enqueue_scripts',array(&$this,'news10_bar_plugin_scripts'));

        add_action('news10_single_navigation',array(&$this,'single_nav'));
        add_action('news10_pagination',array(&$this,'news10_posts_pagination'));
        add_action('news10_related_post',array(&$this,'news10_related_post'));
        add_action('news10_authorbox',array(&$this,'news10_authorbox'));
        add_action('news10_share_tags',array(&$this,'news10_share_tag'));
 
        add_action('news10_header',array(&$this,'news10_render_header'));
        add_action('news10_footer',array(&$this,'news10_render_footer'));
        add_action('news10_sidebar',array(&$this,'news10_render_sidebar'));
        add_action('news10_sidebar_inner',array(&$this,'news10_render_sidebar_inner'));

    }
    function render_preloader(){

        if( news10_options('enb_pre') ){
    ...
    function render_scroll_top(){
        ...
        if( news10_options('enb_scroll') ){
            echo '<a href="#" class="scroll-top"><i class="fa fa-angle-up"></i></a>';
        }
    }

    function news10_render_sidebar(){

        $cust_header = news10_options('sidebar');
        echo do_shortcode('[INSERT_ELEMENTOR id="'.$cust_header.'"]');
        
    }


     function news10_render_sidebar_inner(){

        $cust_header = news10_options('inner_sidebar');
        echo do_shortcode('[INSERT_ELEMENTOR id="'.$cust_header.'"]');

    }
 
    function news10_render_footer(){

        $meta_switch = news10_meta('footer_switch');
        $meta_footer = news10_meta('meta_footer');
    ...
    function news10_render_header(){
        $meta_switch = news10_meta('header_switch');
        $meta_header = news10_meta('meta_header');
    ...
    function news10_share_tag(){

        $tagtitle = news10_options('tag_title');
        $sharetitle = news10_options('share_title');
    ...
    ?>    
        ...
        <div class="share_tag"> 
            <div class="news10_tag float-left text-left ul-li">
                <?php echo $out;?>
            </div>
    ...
    function news10_authorbox(){
        ...
        $title = news10_options('auth_title');
    ...
    function news10_related_post(){
        $title = news10_options('related_title');
        $id = $GLOBALS['post']->ID;
    ...
    function news10_posts_pagination(){
    ...
    function news10_bar_plugin_scripts() {

        function dynamic_css() {
            ob_start();
            include NEWS10_PLUG_DIR . '/vendor/frontend/css.php';
    ...
    function news10_mime_type($mimes) {
